...
class Visualizer(object):
    def __init__(self, resolution=(2048, 2048), renderer_type=None):
        ...
        self.MANO_SKELETON = load_skeleton(path.join(path.dirname(__file__), '../mano/skeleton.txt'), 21)
    ...
    def visualize_renderer_verts_list(self, verts_list, j3d_list, hand_type=None, hand_type_list=None, faces_list=None,
                                      images=None, cam_params=None, \
                                      colors=None, trans=None, thresh=0., pre_colors=np.array(
                [[0.46, 0.59, 0.64], [0.94, 0.71, 0.53]])):  # pre_colors=np.array([[.8, .0, .0], [0, .0, .8]])):
        verts_list = [verts.contiguous() for verts in verts_list]

        if faces_list is None and hand_type is not None:
            faces_list = [self.mano_layer[str(hand_type[0])].th_faces.repeat(len(verts), 1, 1).to(verts.device) for
                          verts in verts_list]
        ...
        if hand_type is not None and hand_type_list is None:
            ...
            for ind, (verts, faces) in enumerate(zip(verts_list, faces_list)):
                ...
                rendered_img = self.renderer(verts, faces, colors=color, focal_length=args().focal_length,
            ...
            for ind, (verts, j3d, hand_type) in enumerate(zip(verts_list, j3d_list, hand_type_list)):
                ...
                '''
                rendered_img = self.renderer(verts, all_face, colors=color, focal_length=args().focal_length,
                                             cam_params=cam_params)
                rendered_imgs.append(rendered_img)
        ...
        if images is not None:
            if not (images.shape == rendered_imgs.shape):
                ...
                images = F.interpolate(torch.from_numpy(images).permute(0, 3, 1, 2).float(),
                                       scale_factor=(scale, scale), mode='bilinear').permute(0, 2, 3, 1).numpy()
    ...
    def visulize_result_live(self, outputs, frame_img, meta_data, show_items=['mesh'], vis_cfg=default_cfg, **kwargs):
        ...
        org_imgs = \
        meta_data['image'][outputs['detection_flag_cache'].to(meta_data['image'].device)].cpu().numpy().astype(
            np.uint8)[img_inds_org]
        ...
        for vis_name in show_items:
            ...
            if vis_name == 'mesh' and outputs['detection_flag']:
                per_img_verts_list = [outputs['verts'][outputs['detection_flag_cache']][inds].detach() for inds in
                                      per_img_inds]
                per_img_j3d_list = [outputs['j3d'][outputs['detection_flag_cache']][inds].detach() for inds in
                                    per_img_inds]
                mesh_trans = [outputs['cam_trans'][outputs['detection_flag_cache']][inds].detach() for inds in
                              per_img_inds]
                hand_type = [
                    outputs['output_hand_type'][outputs['detection_flag_cache'].to(outputs['output_hand_type'].device)][
                        inds].detach() for inds in per_img_inds]

                rendered_imgs = self.visualize_renderer_verts_list(per_img_verts_list, per_img_j3d_list,
                                                                   hand_type_list=hand_type, images=org_imgs.copy(),
                                                                   trans=mesh_trans)
                ...
                if 'put_org' in vis_cfg['settings']:
                    ...
                    for inds, j in enumerate(used_org_inds):
                        ...
                        if args().render_size > 1000:
                            ...
                            org_imge = \
                            F.interpolate(torch.from_numpy(org_imge).unsqueeze(0).permute(0, 3, 1, 2).float(),
                                          scale_factor=(4, 4), mode='bilinear').permute(0, 2, 3, 1).numpy()[0]
                ...
                for img_id, inds_list in enumerate(per_img_inds):
                    ...
                    for kp2d_vis in outputs['pj2d'][inds_list]:
                        if len(kp2d_vis) > 0:
                            ...
                            org_img = self.vis_keypoints(org_img, kp2d_vis.detach().cpu().numpy(),
            ...
            if vis_name == 'centermap' and outputs['detection_flag']:
                ...
                for img_id, (l_centermap, r_centermap) in enumerate(
                        zip(outputs['l_center_map'][used_org_inds], outputs['r_center_map'][used_org_inds])):
